			...
			<div class="content-right fl">
				...
				<div class="best-news">
					<div class="bnews-head">
						<span id="cname" class="sbg fb f16 fl">交通要闻</span>
						<div class="cb"></div>
					</div>

					<c:forEach var="entry" items="${entries}" varStatus="status">
						<c:if test="${entry.importance > 0}">
					...
					<div align="center">
						<a href="keynews.html?category=${category}&pageNum=1">首页</a>

						<c:if test="${currentPage-1 > 0}">
							<a
								href="keynews.html?category=${category}&pageNum=${currentPage-1}">&laquo;</a>
						...
						<c:forEach var="i" begin="${pageBegin}" end="${pageLast}">
							<c:choose>
								<c:when test="${i == currentPage}">
									<a
										href='keynews.html?category=${category}&pageNum=<c:out value="${i}"/>'>
										<c:out value="${i}" />
									</a>
								</c:when>
								<c:otherwise>
						...
						<c:if test="${currentPage+1 < pageCount}">
							<a
								href="keynews.html?category=${category}&pageNum=${currentPage+1}">&raquo;</a>
						</c:if>
						<a href="keynews.html?category=${category}&pageNum=${pageCount}">尾页</a>
					</div>
				</div>			
			</div>
				<div class="cb"></div>
