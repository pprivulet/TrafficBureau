			...
			<div class="content-right fl">
				<div class="content-right-top">
					<span class="dib ml10 f18">新闻中心>></span> <span
						class="f13 div ml405">您现在的位置：首页>双务公开>公告</span>
				</div>
				<div class="best-news">
					<div class="bnews-head">
						<span id="cname" class="sbg fb f16 fl">公告</span>
						<div class="cb"></div>
					</div>
					...
					<div align="center">
						<a href="anounce.html?category=${category}&pageNum=1">首页</a>

						<c:if test="${currentPage-1 > 0}">
							<a
								href="anounce.html?category=${category}&pageNum=${currentPage-1}">&laquo;</a>
						...
						<c:forEach var="i" begin="${pageBegin}" end="${pageLast}">
							<c:choose>
								<c:when test="${i == currentPage}">
									<a
										href='anounce.html?category=${category}&pageNum=<c:out value="${i}"/>'>
										<c:out value="${i}" />
									</a>
								</c:when>
								<c:otherwise>
						...
						<c:if test="${currentPage+1 < pageCount}">
							<a
								href="anounce.html?category=${category}&pageNum=${currentPage+1}">&raquo;</a>
						</c:if>
						<a href="anounce.html?category=${category}&pageNum=${pageCount}">尾页</a>
